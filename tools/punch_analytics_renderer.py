...
def apply_analytics(content):
    ...
    definitions = {
        ...
        'tonys-auto': [('ga4','engagementRate','Engagement rate','percent'),('search_console','position','Average position','position',True),('search_console','ctr','Organic CTR','percent'),('search_console','clicks','Organic clicks')],
    ...
    for slug, fields in definitions.items():
        ...
        if slug == 'tonys-auto':
            narrative = 'Tony’s Auto is attracting a more engaged website audience. Engagement rate increased from 42.0% to 55.3%, a gain of 13.3 percentage points.'
        ...
        if slug == 'tonys-auto':
            card = replace_one(r'<div class="performance-line">.*?</div>',
                '<div class="performance-line"><strong>Traffic quality is moving in the right direction: engagement improved 13.3 percentage points, average position reached 10.6, and organic CTR increased while clicks remained nearly steady.</strong></div>', card)
        ...
        if slug == 'tonys-auto':
            card = card.replace('+13.29 pp vs previous', '+13.3 pp vs previous')
            card = card.replace('-0.63 positions vs previous', 'Improved from 11.2')
            card = card.replace('+0.03 pp vs previous', 'Up from 0.54%')
            card = card.replace('<div class="risk-change">-3.7% vs previous</div>', '<div class="muted-change">Nearly steady · 27 previously</div>')
        tag = 'GA4 only' if slug == 'chem-nut-supply' else ('Organic visibility expanding' if slug == 'punch-transfers' else ('Search visibility surge' if slug == 'phil-medeiros' else ('Traffic momentum' if slug == 'south-coast-towing' else ('Traffic quality improving' if slug == 'tonys-auto' else ('Analytics refreshed' if ga is not None or ads is not None or sc is not None else 'Access pending')))))
    ...
    content = re.sub(r'Performance reporting and account actions across Punch Club\.[^<]+',
        f'Performance reporting and account actions across Punch Club. Reporting period: {period}. Compared with {comparison}. Updated September 7, 2026.\n            ',content,count=1)
    summary = f'The Punch Club portfolio delivered measurable progress across organic search, website engagement, paid reach and lead generation during {period}. The team managed {fmt(sessions)} website sessions, {fmt(search_clicks)} organic search clicks, {fmt(events)} key events and {fmt(revenue, "money")} in GA4-recorded revenue while advancing performance across businesses with distinct audiences and objectives.' if manual_matches else f'The team managed {fmt(sessions)} website sessions across {ga_count} properties for {period}.'
    highlights = (
        '<div class="executive-highlights">'
        '<div class="executive-highlight"><strong>Search visibility</strong><p>Phil Medeiros reached an average page-one position, with clicks increasing 125% and impressions increasing 1,624%. Punch Transfers grew page-level impressions 82%. South Coast Towing and Tony’s Auto also improved their average search positions.</p></div>'
        '<div class="executive-highlight"><strong>Demand generation</strong><p>Grub Tub generated 50,835 Meta impressions, 3,577 clicks and 1,868 landing-page views at an $0.08 CPC. Modern Auto Body produced five Instant Form leads at $25.98 each. Kathy Mackenzie recorded 764 campaign clicks at a 3.85% CTR, while LC Mechanical increased GA4 key events 60%.</p></div>'
        '<div class="executive-highlight"><strong>Audience quality</strong><p>Chem Nut Supply increased sessions 26.2% and active users 24.9%. South Coast Towing increased sessions 8.3% and active users 14.4%. Tony’s Auto raised engagement from 42.0% to 55.3%, and Punch Creatives improved sessions, active users, engagement and organic clicks.</p></div>'
        '</div>'
    ) if manual_matches else ''
    priority = '<p class="executive-priority"><strong>Executive priority:</strong> Convert expanding visibility and stronger audience engagement into measurable calls, forms, store clicks, bookings and revenue. The next phase centers on conversion tracking, high-opportunity search pages and consistent lead follow-through.</p>' if manual_matches else ''
    ...
    content = replace_one(r'(<section class="card wide-card" aria-labelledby="executive-summary-title">).*?</section>',
        '<section class="card wide-card" aria-labelledby="executive-summary-title"><h2 id="executive-summary-title">Executive Portfolio Update</h2>'
